...
var MMLBuilder = function(redis_pool, opts, optional_args, init_callback){
    ...
    // core variables
    if (!opts || !opts.hasOwnProperty('dbtype'))
        throw new Error("Options must include dbtype");
    ...
    var grainstore_defaults = {
        ...
        datasource: {
            ...
            //password: kept default
            //table: kept default
            geometry_field: "the_geom_webmercator",
            //geometry_table: kept default
            srid: 3857,
            extent: "-20037508.3,-20037508.3,20037508.3,20037508.3",
    ...
    me.baseMML = function(args){
        ...
        for (var i=0; i<tables.length; ++i) {
          ...
          if(datasource.table && datasource.table != table) {
			throw new Error('"'+table + '" table does not match "'+datasource.table+'" optional options table');
          }  else {
			 datasource.table   = table;
          }

          if(datasource.dbname && datasource.dbname != opts.dbname) {
			throw new Error('"'+opts.dbname + '" dbname does not match "'+datasource.dbname+'" optional datasource option dbname');
          }  else {
			 datasource.dbname  = opts.dbname;
          }

		// Note: datasource type is what we consider dbtype. Assumption all datasource types are databases
          if(datasource.type && datasource.type != opts.dbtype) {
			throw new Error('"'+opts.dbtype + '" dbtype does not match "'+datasource.type+'" optional options type');
          }  else {
			datasource.type  = opts.dbtype;
          }
