...
//
// opts may have:
// `dbtype`          - type of database (defaulting to 'postgis')
...
function MMLBuilder(params, options) {
    ...
    // Set default dbtype
    this.params.dbtype = params.dbtype || 'postgis';

    // core variables
    var requiredParams = ['dbtype', 'dbname', 'sql', 'style'];
    ...
    this.options.cachedir = this.options.cachedir || os.tmpdir() + '/millstone/';
    ...
    if(!this.params.style) {
        this.params.style = 		
        '#' + this.params.table + '[mapnik-geometry-type=1]' + def_style_point +		
        '#' + this.params.table + '[mapnik-geometry-type=2]' + def_style_line +		
        '#' + this.params.table + '[mapnik-geometry-type=3]' + def_style_poly;
    }
    
    this.grainstore_datasource = _.defaults(_.clone(options.datasource || {}), grainstore_defaults.datasource);
    this.grainstore_properties = _.defaults(_.clone(options.properties || {}), grainstore_defaults.properties);
...
MMLBuilder.prototype.baseMML = function() {
    ...
    for (var i=0; i<tables.length; ++i) {
        ...
        datasource.type = this.params.dbtype;
        ...
        if ( this.params.gcols && this.params.gcols[i] ) {
            if (_.isString(this.params.gcols[i])) {
                ...
                if (dsOpts && gcol.name) {
                    ...
                    datasource.type = (dsOpts.type == 'database') ? this.params.dbtype : dsOpts.type;
        ...
        var properties = _.clone(this.grainstore_properties);
...
MMLBuilder.prototype.toMML = function(style_in){
    ...
    for (var i=0; i<style.length; ++i) {
        ...
        if ( _.isArray(style_in) ) {
            ...
            stylesheet.data = t.setLayerName(style[i], base_mml.Layer[i].id);
        } else {
